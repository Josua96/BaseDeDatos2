...
/**
 Triggers sobre tablas accidentes, accidentes generales,accidentes personas,heridos y fallecidos

*/ 

CREATE OR REPLACE FUNCTION validar_insertUpAccidentes()
RETURNS TRIGGER
AS 
$BODY$
BEGIN
	IF (TG_RELNAME='accidentes') THEN
		IF ((SELECT COUNT(*) FROM DetallesAcidentes.TiposLesiones 
		WHERE Id=NEW.IdTipoLesion) =0) THEN
		RAISE EXCEPTION 'Insercion no permitida';

		END IF;
	ELSIF (TG_RELNAME='accidentespersonas') THEN
		IF(((SELECT COUNT(*) FROM AccidentesTran.Accidentes WHERE Id=NEW.IdAccidente)=0)OR
		((SELECT COUNT(*) FROM DetallesAccidentes.RolesPersonas 
		WHERE Id=NEW.IdRolPersona)=0)) THEN 
			RAISE EXCEPTION 'Insercion no permitida';

		END IF;
	ELSIF (TG_RELNAME='heridos') THEN
		IF (((SELECT COUNT(*) FROM AccidentesTran.Accidentes WHERE Id=NEW.IdAccidente)=0)
		OR ((SELECT COUNT(*) FROM Direccion.Distritos WHERE IdDistrito=NEW.IdDistrito)=0)
		) THEN
		RAISE EXCEPTION 'Insercion no permitida';

		END IF;
		
	ELSIF (TG_RELNAME='fallecidos') THEN
		IF (((SELECT COUNT(*) FROM AccidentesTran.Accidentes WHERE Id=NEW.IdAccidente)=0)OR
		  ((SELECT COUNT(*) FROM Direccion.Cantones WHERE IdCanton=NEW.IdCanton)=0)OR
		  ((SELECT COUNT(*) FROM DetallesAccidentes.TiposAccidentes 
		  WHERE Id=NEW.IdTipoAccidente)=0) OR
		  ((SELECT COUNT(*) FROM DetallesAccidentes.KilometrosRutas 
		  WHERE Id=NEW.IdRuta)=0) ) THEN
		  RAISE EXCEPTION 'Insercion no permitida';

		END IF;
	ELSE   
		IF (((SELECT COUNT(*) FROM AccidentesTran.Accidentes WHERE Id=NEW.IdAccidente)=0)OR
		   ((SELECT COUNT(*) FROM Direccion.Distritos WHERE IdDistrito=NEW.IdDistrito)=0)OR
		   ((SELECT COUNT(*) FROM DetallesAccidentes.TiposCirculacion 
		   WHERE Id=NEW.IdTipoCirculacion)=0)OR
		   ((SELECT COUNT(*) FROM DetallesAccidentes.EstadosTiempo
		   WHERE Id=NEW.IdEstadoTiempo)=0) OR
		   ((SELECT COUNT(*) FROM DetallesAccidentes.TiposCalzadas
		   WHERE Id=NEW.IdTipoCalzada)=0) OR
		   ((SELECT COUNT(*) FROM DetallesAccidentes.DescripcionesCalzadas
		   WHERE Id=NEW.IdDescripcionCalzadaVertical OR 
		   Id=NEW.IdDescripcionCalzadaHorizontal)<2) OR
		   ((SELECT COUNT(*) FROM DetallesAccidentes.TiposAccidentes 
		   WHERE Id=NEW.IdTipoAccidente)=0)OR
		   ((SELECT COUNT(*) FROM DetallesAccidentes.KilometrosRutas 
		   WHERE Id=NEW.IdKilometro OR Id=NEW.IdRuta)<2)
		) THEN
		RAISE EXCEPTION 'Insercion no permitida';
...
BEFORE INSERT
ON AccidentesTran.Accidentes
FOR EACH ROW
EXECUTE PROCEDURE validar_insertUpAccidentes();

CREATE TRIGGER trigger_updateAccidentes
AFTER UPDATE
ON AccidentesTran.Accidentes
FOR EACH ROW
EXECUTE PROCEDURE validar_insertUpAccidentes();

CREATE TRIGGER trigger_insertAccidentesPersonas
BEFORE INSERT
ON AccidentesTran.AccidentesPersonas
FOR EACH ROW
EXECUTE PROCEDURE validar_insertUpAccidentes();

CREATE TRIGGER trigger_updateAccidentesPersonas
AFTER UPDATE
ON AccidentesTran.AccidentesPersonas
FOR EACH ROW
EXECUTE PROCEDURE validar_insertUpAccidentes();

CREATE TRIGGER trigger_insertHeridos
BEFORE INSERT
ON AccidentesTran.Heridos
FOR EACH ROW
EXECUTE PROCEDURE validar_insertUpAccidentes();

CREATE TRIGGER trigger_UpdateHeridos
AFTER UPDATE
ON AccidentesTran.Heridos
FOR EACH ROW
EXECUTE PROCEDURE validar_insertUpAccidentes();

CREATE TRIGGER trigger_insertFallecidos
BEFORE INSERT
ON AccidentesTran.Fallecidos
FOR EACH ROW
EXECUTE PROCEDURE validar_insertUpAccidentes();

CREATE TRIGGER trigger_updateFallecidos
AFTER UPDATE
ON AccidentesTran.Fallecidos
FOR EACH ROW
EXECUTE PROCEDURE validar_insertUpAccidentes();

CREATE TRIGGER trigger_insertAccidentesGenerales
BEFORE INSERT
ON AccidentesTran.AccidentesGenerales
FOR EACH ROW
EXECUTE PROCEDURE validar_insertUpAccidentes();

CREATE TRIGGER trigger_updateAccidentesGenerales
AFTER UPDATE
ON AccidentesTran.AccidentesGenerales
FOR EACH ROW
EXECUTE PROCEDURE validar_insertUpAccidentes();
